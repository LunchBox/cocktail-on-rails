class MarksController < ApplicationController
  ...
  def create
		@markable = params[:markable_type].constantize.find(params[:markable_id])
    @mark = Mark.new
		if Mark::CONTEXTS.include? params[:context]
			@mark.context = params[:context]
		end
    ...
    respond_to do |format|
      if @mark.save
        format.html { redirect_back fallback_location: @markable, notice: "Mark was successfully created." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_back fallback_location: @mark.markable, notice: "Mark was successfully destroyed." }
  ...
  private
    ...
    def mark_params
      params.require(:mark).permit(:context)
    end
end
